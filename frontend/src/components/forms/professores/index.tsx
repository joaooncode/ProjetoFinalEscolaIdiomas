...
import { useCreateProfessorForm, type CreateProfessorFormData } from "../schemas/professorSchema";
import {
    Form,
    FormControl,
    FormField,
...
interface CreateProfessorFormProps {
    onSubmit: (data: CreateProfessorFormData) => void;
    isLoading?: boolean;
    onCancel?: () => void;
}

export const CreateProfessorForm: React.FC<CreateProfessorFormProps> = ({
    onSubmit,
    isLoading = false,
    onCancel,
}) => {
    const form = useCreateProfessorForm();

    const handleSubmit = (data: CreateProfessorFormData) => {
        onSubmit(data);
    };
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription className="text-xs text-muted-foreground flex items-center gap-1">
                            ...
                            O professor deve ter pelo menos 18 anos de idade
                          </FormDescription>
                          <FormMessage className="text-destructive text-sm" />
                        </FormItem>
                      )}
                ...
                <div className="bg-card rounded-xl shadow-lg border border-border p-6">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Dados de formação e qualificação
                      </p>
                    </div>
                  </div>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <Select onValueChange={field.onChange} defaultValue={field.value} disabled={isLoading}>
                            ...
                            <SelectContent>
                              {Object.entries(Formacoes).map(([key, value]) => (
                        ...
                        <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-lg border border-border p-4 bg-muted/50">
                          ...
                          <div className="space-y-1 leading-none">
                            ...
                            <FormDescription className="text-xs text-muted-foreground">
                              Marque se o professor está ativo no sistema
                            </FormDescription>
                          </div>
                        </FormItem>
                      )}
